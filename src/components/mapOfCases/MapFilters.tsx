...
export function MapFilters({ currentFilters, onFiltersChange }: MapFiltersProps) {
  const [selectedPeriod, setSelectedPeriod] = useState(currentFilters?.dateFilter || 'lastWeek')
  const [selectedRisk, setSelectedRisk] = useState(currentFilters?.riskLevel || 'all')
  ...
  const periods = [
    ...
    { value: "1year" as const, label: "Último año" },
  ...
  const getSelectedPeriodLabel = () => {
    return periods.find(p => p.value === selectedPeriod)?.label || "Seleccionar período"
  }

  const getSelectedRiskLabel = () => {
    return riskLevels.find(r => r.value === selectedRisk)?.label || "Seleccionar nivel"
  }
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm">
      <div className="p-6 border-b border-gray-200">
        ...
        <p className="text-sm text-gray-600 mt-1">Personaliza la visualización de datos en el mapa</p>
      </div>
      <div className="p-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={periodRef}>
              ...
              >
                ...
                <ChevronDown className={`h-4 w-4 text-gray-400 transition-transform ${isPeriodOpen ? 'rotate-180' : ''}`} />
              </button>
                  ...
                  {periods.map((period) => (
                    <button
                      ...
                      className={`w-full px-3 py-2 text-left text-sm hover:bg-gray-100 transition-colors ${
                        selectedPeriod === period.value ? 'bg-[var(--color-primary)]/10 text-[var(--color-primary)]' : 'text-gray-900'
                      }`}
                    >
                      {period.label}
                    </button>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative" ref={riskRef}>
              ...
              >
                ...
                <ChevronDown className={`h-4 w-4 text-gray-400 transition-transform ${isRiskOpen ? 'rotate-180' : ''}`} />
              </button>
                ...
                <div className="absolute top-full mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg z-50">
                  {riskLevels.map((risk) => (
                    <button
                      ...
                      className={`w-full px-3 py-2 text-left text-sm hover:bg-gray-100 transition-colors ${
                        selectedRisk === risk.value ? 'bg-[var(--color-primary)]/10 text-[var(--color-primary)]' : 'text-gray-900'
                      }`}
                    >
                      {risk.label}
                    </button>
